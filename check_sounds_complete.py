...
expected_sounds = {
    ...
    "creature": [
        ...
        "scorpion_attack",
        "harpy_attack",
        "giant_serpent_attack",
        "lizardman_attack",
        "orc_attack",
        "ratman_attack",
        "goblin_attack",
        "gargoyle_attack",
        "stone_gargoyle_attack",
        "troll_attack",
        "ogre_attack",
        "ettin_attack",
        "cyclops_attack",
...
print(
    f"Coverage: {total_generated / (total_expected * 1.5) * 100:.1f}% (accounting for variations)"
)
print()
...
for cat in sorted(expected_sounds.keys()):
    ...
    print(
        f"{status} {cat:12s}: {generated_count:3d} files (expected ~{expected_count} concepts)"
    )
